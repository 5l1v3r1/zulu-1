...
int iwconfig_set_ssid(const char *in_dev, char *errstr, char *in_essid) {
    ...
    if ((skfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to create ioctl socket to set SSID on %s: %s", 
    ...
    if (ioctl(skfd, SIOCSIWESSID, &wrq) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to set SSID on %s: %s",  
                 in_dev, strerror(errno));
        close(skfd);
        return -1;
    }
...
int iwconfig_get_ssid(const char *in_dev, char *errstr, char *in_essid) {
    ...
    if ((skfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to create socket to fetch SSID on %s: %s", 
                 in_dev, strerror(errno));
        return -1;
    }
    ...
    if (ioctl(skfd, SIOCGIWESSID, &wrq) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to fetch SSID from %s: %s", 
                 in_dev, strerror(errno));
        close(skfd);
        return -1;
    }
...
int iwconfig_get_name(const char *in_dev, char *errstr, char *in_name) {
    ...
    if ((skfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to create socket to get name on %s: %s",
                 in_dev, strerror(errno));
        return -1;
    }
    ...
    if (ioctl(skfd, SIOCGIWNAME, &wrq) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to get name on %s :%s", in_dev,
                 strerror(errno));
        close(skfd);
        return -1;
    }
...
int iwconfig_set_intpriv(const char *in_dev, const char *privcmd, 
    ...
    if ((skfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to create socket to set private ioctl on %s: %s",
                 in_dev, strerror(errno));
        return -1;
    }
    ...
    if (ioctl(skfd, SIOCGIWPRIV, &wrq) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to retrieve list of private ioctls on %s: %s",
                 in_dev, strerror(errno));
        close(skfd);
        return -1;
    }
    ...
    if (pn == wrq.u.data.length) {
        snprintf(errstr, TX80211_STATUS_MAX, "Unable to find private ioctl '%s' on %s", 
                 privcmd, in_dev);
        close(skfd);
        return -2;
    }
    ...
    if (priv[pn].cmd < SIOCDEVPRIVATE) {
        ...
        if (j == wrq.u.data.length) {
            snprintf(errstr, TX80211_STATUS_MAX, "Unable to find subioctl '%s' on %s", 
                     privcmd, in_dev);
            close(skfd);
            return -2;
        }
    ...
    if ( (priv[pn].set_args & IW_PRIV_TYPE_MASK) == 0 ||
        ...
        snprintf(errstr, TX80211_STATUS_MAX, "Unable to set values for private ioctl '%s' on %s", 
                 privcmd, in_dev);
        close(skfd);
        return -1;
    }
    ...
    if ((priv[pn].set_args & IW_PRIV_TYPE_MASK) != IW_PRIV_TYPE_INT) {
        snprintf(errstr, TX80211_STATUS_MAX, "'%s' on %s does not accept integer parameters.",
                 privcmd, in_dev);
        close(skfd);
        return -1;
    }
    ...
    if (nargs > 2) {
        snprintf(errstr, TX80211_STATUS_MAX, "Private ioctl '%s' on %s expects more than "
    ...
    if (ioctl(skfd, priv[pn].cmd, &wrq) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to set private ioctl '%s' on %s: %s",
...
int iwconfig_get_intpriv(const char *in_dev, const char *privcmd,
    ...
    if ((skfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to create socket to fetch private ioctl on %s: %s",
                 in_dev, strerror(errno));
        return -1;
    }
    ...
    if ( (priv[pn].get_args & IW_PRIV_TYPE_MASK) == 0 ||
        ...
        snprintf(errstr, TX80211_STATUS_MAX, "Unable to get values for private ioctl '%s' on %s", 
                 privcmd, in_dev);
        close(skfd);
        return -1;
    }
    ...
    if ((priv[pn].get_args & IW_PRIV_TYPE_MASK) != IW_PRIV_TYPE_INT) {
        snprintf(errstr, TX80211_STATUS_MAX, "Private ioctl '%s' on %s does not return "
    ...
    if (nargs > 1) {
        snprintf(errstr, TX80211_STATUS_MAX, "Private ioctl '%s' on %s returns more than 1 "
    ...
    /* Actually do it. */
    if (ioctl(skfd, priv[pn].cmd, &wrq) < 0) {
        snprintf(errstr, TX80211_STATUS_MAX, "Failed to call get private ioctl '%s' on %s: %s",
                 privcmd, in_dev, strerror(errno));
        close(skfd);
        return -1;
    }
...
int iwconfig_get_levels(const char *in_dev, char *in_err, int *level, int *noise) {
    ...
    if ((skfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        snprintf(in_err, TX80211_STATUS_MAX, "Failed to create AF_INET DGRAM socket %d:%s", 
    ...
    if (ioctl(skfd, SIOCGIWRANGE, &wrq) < 0) {
        snprintf(in_err, TX80211_STATUS_MAX, "Failed to fetch signal range, %s", strerror(errno));
        close(skfd);
        return -1;
    }
    ...
    if (ioctl(skfd, SIOCGIWSTATS, &wrq) < 0) {
        snprintf(in_err, TX80211_STATUS_MAX, "Failed to fetch signal stats, %s", strerror(errno));
        close(skfd);
        return -1;
    }
...
int iwconfig_get_channel(const char *in_dev, char *in_err) {
    ...
    if (ioctl(skfd, SIOCGIWFREQ, &wrq) < 0) {
        snprintf(in_err, TX80211_STATUS_MAX, "channel get ioctl failed %d:%s",
                 errno, strerror(errno));
        close(skfd);
        return -1;
    }
...
int iwconfig_set_channel(const char *in_dev, char *in_err, int in_ch) {
    ...
    if (ioctl(skfd, SIOCSIWFREQ, &wrq) < 0) {
        ...
        if (ioctl(skfd, SIOCSIWFREQ, &wrq) < 0) {
            snprintf(in_err, TX80211_STATUS_MAX, "Failed to set channel %d %d:%s", in_ch,
                     errno, strerror(errno));
            close(skfd);
            return -1;
        }
...
int iwconfig_get_mode(const char *in_dev, char *in_err) {
    ...
    if (ioctl(skfd, SIOCGIWMODE, &wrq) < 0) {
        snprintf(in_err, TX80211_STATUS_MAX, "mode get ioctl failed %d:%s",
                 errno, strerror(errno));
        close(skfd);
        return -1;
    }
...
int iwconfig_set_mode(const char *in_dev, char *in_err, int in_mode) {
    ...
    if (ioctl(skfd, SIOCSIWMODE, &wrq) < 0) {
        snprintf(in_err, TX80211_STATUS_MAX, "mode set ioctl failed %d:%s",
                 errno, strerror(errno));
        close(skfd);
        return -1;
    }
